...
import { satisfied_product_devolution } from '@CVApi/core/webdevolucion/DevService';
function DVCRecepcion({ isOpen, onClose, process, iddevolucion }) {
  const [loading, setloading] = useState(false);
  const addToast = useToast();

  const senddata = async () => {
    setloading(true);
    const result = await satisfied_product_devolution(iddevolucion);
    if (result) {
      await process();
    } else {
  ...
  return (
    <CVModal
      ...
      <CVText fontWeight='bold' color='blue' textAlign='center'>
        ¿Estás conforme con el producto devuelto?
      </CVText>
    </CVModal>
  );
}

export default DVCRecepcion;
